...
type NoteEditorProps =
  | { mode: "create"; orgTags: Tag[]; templates?: Template[] }
  | {
      mode: "edit";
      note: { id: string; title: string; content: string; visibility: Visibility; tagIds: string[]; isAuthor: boolean; updatedAt: string };
      orgTags: Tag[];
      templates?: Template[];
    };
    ...
    try {
      if (isEdit) {
        await updateNote({
          ...
          expectedUpdatedAt: props.note.updatedAt,
        });
        ...
        {templates.length > 0 && !isEdit && (
          <div className="relative">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 border border-[var(--border-color)] rounded-card text-[12px] text-dim hover:bg-subtle hover:text-ink transition-colors"
            >
            ...
            {showTemplates && (
              <div className="absolute right-0 top-full mt-1.5 w-64 bg-surface border border-[var(--border-color)] rounded-card shadow-card z-10">
                <div className="px-3 py-2 border-b border-[var(--border-color)]">
                  <p className="text-[11px] font-semibold text-muted uppercase tracking-wider">Templates</p>
                </div>
                <div className="py-1 max-h-60 overflow-y-auto">
                  {templates.map((t) => (
                    <button
                      ...
                      className="w-full text-left px-3 py-2 text-[13px] text-ink hover:bg-subtle transition-colors"
                    >
                      <p className="font-medium truncate">{t.title}</p>
                      {t.content && (
                        <p className="text-[11px] text-muted truncate mt-0.5">{t.content.slice(0, 60)}</p>
                      )}
                    </button>
                  ))}
                </div>
